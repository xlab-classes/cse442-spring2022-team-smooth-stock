...
def return_support_page():
    ...
    response_hopefully_text = response.content
    # print(response)

    # print(response_hopefully_text)
    int = response_hopefully_text.find(b'ask')
    build = b''
    for i in range(int, int + 11):
        build += response_hopefully_text[i:i + 1]

    # print(int)
    # print(build)
    str_build = str(build)
    str_build_ = str_build[7:len(str_build) - 1]
    return render_template('support.html', utc_dt=str_build_)
